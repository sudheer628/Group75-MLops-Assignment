...
Design, develop, and deploy a scalable and reproducible machine learning solution utilising modern
MLOps best practices. The assignment emphasises practical automation, experiment tracking, CI/CD
pipelines, containerization, cloud deployment, and monitoring—mirroring real-world production
scenarios.

### Dataset:

Heart Disease UCI Dataset (code available in import_dataset.py)
...
3. Experiment Tracking [5 marks]
   Integrate MLflow for experiment tracking. Log parameters, metrics, artifacts, and plots for all runs.
...
5. CI/CD Pipeline & Automated Testing [8 marks]
   Write unit tests for data processing and model code (Pytest or unit test). Create a GitHub Actions pipeline That Includes Linting, unit testing, and model training steps.Artifacts/logging for each workflow run.

6. Model Containerization [5 marks]
   Build a Docker container for the model-serving API (FastAPI is recommended). Expose /predict endpoint, accept JSON input, return prediction and confidence. The container must be built and run locally with the sample input.

7. Production Deployment [7 marks]
   Deploy the Dockerized API to a public cloud or local Kubernetes (EKS or Minikube/Docker Desktop).
...
- Short video containing an end-to-end pipeline
