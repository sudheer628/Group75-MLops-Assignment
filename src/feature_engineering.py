...
def perform_cross_validation(models, X_train, y_train, cv_folds=5):
    ...
    for name, model in models.items():
        ...
        pipeline = Pipeline([("preprocessor", create_preprocessing_pipeline()), ("classifier", model)])

        # Perform cross-validation
        cv_scores = cross_val_score(pipeline, X_train, y_train, cv=cv, scoring="roc_auc")

        cv_results[name] = {"scores": cv_scores, "mean": cv_scores.mean(), "std": cv_scores.std(), "pipeline": pipeline}
...
def hyperparameter_tuning(best_models, X_train, y_train):
    ...
    for name, model_info in best_models.items():
        if name in param_grids:
            ...
            tuned_models[name] = {"model": model_info["pipeline"], "best_score": model_info["mean"]}
...
def save_models_and_results(evaluation_results, target_info, feature_names):
    ...
    results_summary = {}
    for name, results in evaluation_results.items():
        results_summary[name] = {"metrics": results["metrics"], "classification_report": results["classification_report"]}
...
def main():
    ...
    try:
        ...
        top_models = dict(list(sorted(cv_results.items(), key=lambda x: x[1]["mean"], reverse=True))[:3])
